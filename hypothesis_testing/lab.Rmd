...
With that in place, we're going to perform very many tests, varying the probabilities of association and the size of the cohort. We're going to count how many times we've discovered a "significant association" between genotype and phenotype. For this, we build a little function that gathers all pvalues and scores them as signficant or nonsignificant:
...
You can see that the density is approximately uniform. How many times are we discovering a "significant association" (though we know there is none)? This is the proportion of Type I errors:

```{r}
mean(dt_equal$significant)
```
About 5% of the cases, as expected. Now let's give gentotype A a slightly higher incidence and see what happens to the pvalues:
...
```
We're almost always right (we are making very few Type II errors). How is this dependent on the size of the cohort of patients we're examining? Let's use a much smaller size:
...
We have have a huge False Discovery Rate: of the 205 SNPs detected in our analysis, only 8 are actually linked to the disease.

Let's repeat this, using a higher prior probability---now 10% of the SNPs are linked with the disease:
```{r}
...
You can see that we're doing better. The problem of weeding false positives is more difficult when we're looking for a needle in a haystack. For GWAS and similar problems, we need to walk a tight rope:
...
- In practice, scientists have devised statistical techniques to strongly cap Type I errors (considered somehow "worse"), and then control Type II errors as best as they can.

# Correcting for multiple comparisons

As we've seen, if the null hypothesis were true, then about 5% of the pvalues would be below 0.05. As such, if we are scanning many SNPs, we would call 5% of them significantly associated with the disease even when none of them are. Suppose that 1% of the SNPs are strongly linked with the disease, while 99% are not, and that we have 10000 genes. For the truly linked SNPs, we sample p-values between 0 and 0.0005 at random; for the unlinked we sample pvalues between 0 and 1:
```{r}
...
About 5 in 6 are false discoveries. One way to reduce Type I errors is to control for the *Family Wise Error Rate*, rather than controlling each test separately. For example, Bonferroni correction takes pvalues and multiply them by the number of tests:

$p' = min(p n, 1)$

In `R`, you can correct pvalues using
```{r}
...
But as you can see we have thown the baby with the bathwater! There are more sophisticated corrections, that can try walk the tight rope:
```{r}
...
Not bad! This example shows that controlling for multiple testing is essential when performing this type of analysis (lest ending up with a dead salmon result). You can read about the problem of multiple comparisons [here](https://www.stat.berkeley.edu/~mgoldman/Section0402.pdf).
