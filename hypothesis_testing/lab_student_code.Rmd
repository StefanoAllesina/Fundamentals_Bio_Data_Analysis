...
build_data <- function(probA = 0.1, probB = 0.1, size){
  ...
  #     Has_disease Does_not
  #  A     A yes     A no
  #  B     B yes     B no
  # #################################################
  # assume that there are "size" patients for each 
  # phenotype and that association is random with
...
gwas_simulator <- function (test_specificity, test_sensitivity, prior, num_snps){
  # Input:
  # test_sensitivity: 
  # probability of calling the SNP when it is associated with the disease
  # test_specificity:
  # probability of not calling the SNP when it is not associated with the disease
  # prior:
  # frequency of association between SNPs and disease
  # assume that each SNP has prob. prior of being associated
  # and that they are independent
  # num_snps:
  # number of SNPs
  #
  # Outline:
  # for each SNP, determine whether it's associated with the disease (using prior)
  # if it is associated, then it is TRUE POSITIVE w.p. test_sensitivity;
  # otherwise it is a FALSE NEGATIVE
  # if it is not associated, then it is TRUE NEGATIVE w.p. test_specificity;
  # otherwise it is a FALSE POSTIVE
  # Return False Discovery Rate: FP / (FP + TP)
  # 
  # Notes: there's an obvious way to do this using a for loop
  # try to do using vectors to make it faster
  return (FP / (TP + FP))
}
