...
```{r}
# The "grand mean" 
diet %>% summarise(mean = mean(weight.loss), var = var(weight.loss))

by_diet <- diet %>% group_by(Diet) %>% summarise(mean = mean(weight.loss), var = var(weight.loss))

head(by_diet)

var_between <- var(by_diet$mean)
print(paste("The variance between groups is ", var_between))

var_within <- mean(by_diet$var)
