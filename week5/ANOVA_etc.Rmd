...
Denote the set of means of $k$ different groups to be $\{\bar Y_i\}$ and the "grand mean" to be $\mathbf{\bar Y}$ and the *between group variability* is define to be:
$$  S^2_b = \frac{1}{k-1} \sum_i n_i (\bar Y_i -\mathbf{\bar Y})^2 $$
and this is compared to the *within group variabiity*, which if we denote the set of measurements in group $i$ to be $\{y_{ij}\}$, is defined as
$$
...
2. Run ANOVA for pay as dependent on the two factors, report the variance between means and the variance within groups, and the p-value for the null hypothesis.
```{r}
