...
ANOVA is a method for testing the hypothesis that there is no difference in means of subsets of measurements grouped by factors. 
...
Denote the set of means of $k$ different groups to be $\{\bar Y_i\}$ and the "grand mean" to be $\mathbf{\bar Y}$ and the variance of the means, also called the mean square differences for treatments is defined as:
...
$$
S^2_w = \frac{1}{N-k} \sum_j \sum_i (\bar Y_i - y_{ij})^2
$$
...
```{r}
# The "grand mean" 
grand_mean <- mean(diet$weight.loss)
print(paste("The grand mean is", grand_mean))
grand_var <- var(diet$weight.loss)
print(paste("The grand variance is", grand_var))
diet1 <- filter(diet, Diet == 1)
diet2 <- filter(diet, Diet == 2)
diet3 <- filter(diet, Diet == 3)
mean1 <- mean(diet1$weight.loss)
mean2 <- mean(diet2$weight.loss)
mean3 <- mean(diet3$weight.loss)
n1 <- length(diet1$weight.loss)
n2 <- length(diet2$weight.loss)
n3 <- length(diet3$weight.loss)
var_between <- var(c(mean1, mean2, mean3))
print(paste("The variance between groups is ", var_between))
var1 <- var(diet1$weight.loss)
var2 <- var(diet2$weight.loss)
var3 <- var(diet3$weight.loss)
var_within <- (n1*var1 + n2*var2 + n3*var3)/(n1+n2+n3 - 3)
...
## Questions to investigate for a new dataset

1. Load the dataset, explore the variables, plot their distributions by factor, report what you think.
```{r}

```

2. Run ANOVA, report the variance between means and the variance within groups, and the p-value for the null hypothesis.
```{r}
...
4. If the data set has multiple factors, run a two-way ANOVA and provide interpretation.
```{r}
