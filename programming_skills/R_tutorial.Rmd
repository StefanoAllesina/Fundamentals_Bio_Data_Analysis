...
ch6 <- read.table("https://tinyurl.com/y7vctq3v", header = TRUE, stringsAsFactors = FALSE)
```
...
During the class, we will write a lot of code, of increasing complexity. Here is what you should do to write programs that are well-organized, easy to understand, and easy to debug.
...
For example, let's write code that takes the data on Chromosome 6 we have seen above, and try to identify which SNPs deviate the most from Hardy-Weinberg equilibrium. Remember that in an infinite population, where mating is random, there is no selection and no mutations, then the proportion of people carrying the alleles $A1A1$ should be approximately $p_{11} = p^2$ (where $p$ is the frequency of the first allele in the population), those carrying $A1A2$ should be $p_{12} = 2 p q$ (where $q = 1-p$) and finally those carrying $A2A2$ should be $p_{22} = q^2$. This is called the Hardy-Weinberg equilibrium.
...
To find those with the largest deviations, we can compute for each:
