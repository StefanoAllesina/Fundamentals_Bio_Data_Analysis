...
> **Exercise:** 
> 
> Why are two equal signs (`==`) used to check that two numbers are equal? What happens if you use only one sign?
...
If you want to know more about a function, type `?my_function_name` in the console (e.g., `?abs`). This will open the help page in one of the panels on the right.

Stackoverflow is also great for help on `R`. 
...
ch6 <- read.table("https://tinyurl.com/y7vctq3v", header = TRUE)
```
...
This is a good place to pause and introduce how to go about writing programs that are well-organized, easy to write, and easy to debug.
