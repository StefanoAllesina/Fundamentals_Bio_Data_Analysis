...
The same approach can be taken whenever you have categorical values: `R` will automatically create **dummy variables** each encoding whether the ith data point belongs to a particular category. For example, suppose you want to predict height based on the height of the father, and that you also collected the gender, in three categories: `F` for female, `M` for male, `U` for unknown. Then you could use this information to build the model:
$$
...
However, suppose that we believe the type of diet to affect differentially men and women. Then, we would like to create an "interaction" (e.g., vegan-female, paleo-man):
